...
import { USER_STORAGE_KEY } from '../../controller/ApiController';
...
export default class MovieListScreen extends Component {
  ...
  async componentDidMount() {
    AsyncStorage.getItem(USER_STORAGE_KEY).then((username) => this.setState({ username }));
    try {
    ...
    if (totalPages !== page && results.length > 0) {
      return (
        <View style={styles.loadingMore}>
          ...
          >
            <Text style={styles.loadingText}>Cargar Mas</Text>
          </TouchableOpacity>
        </View>
      );
    }
